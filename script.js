...
const fortunes = [
    ...
    "It is honorable to stand up for that which is right.",
    ...
    "Start working on an artistic hobby. Soon it will pay off.",
    ...
    "Take care and sensitivity you show others will return to you.",
    "Take the high road.",
...
getFortuneBtn.addEventListener('click', () => {
    ...
    if (name) {
        ...
        fortuneText.textContent = `${name}, ${randomFortune}`;
        fortuneResult.classList.add('show');
    }
});

// Get fortune on Enter key
...
// --- Sparkle/Glitter Effect on Mouse Movement ---
function createSparkle(x, y, parent) {
    const sparkle = document.createElement('span');
    sparkle.className = 'sparkle';
    sparkle.style.left = x + 'px';
    sparkle.style.top = y + 'px';
    sparkle.style.setProperty('--sparkle-hue', Math.floor(Math.random() * 360));
...
if (heroImage && heroSection) {
    heroSection.addEventListener('mousemove', e => {
        ...
        heroImage.style.transform = `translate(${x * 24}px, ${y * 18}px) scale(1.04)`;
    });
...
// --- Section Reveal on Scroll (already present, but ensure all .glass-card) ---
document.querySelectorAll('section, .glass-card').forEach(section => {
    observer.observe(section);
});

// --- Sparkle CSS Injection (if not present) ---
if (!document.getElementById('sparkle-style')) {
    ...
    .sparkle {
        ...
        height: 14px;
        border-radius: 50%;
        background: conic-gradient(from 0deg, hsl(var(--sparkle-hue),100%,70%) 0%, #fff 60%, transparent 100%);
        box-shadow: 0 0 8px 2px #fff7, 0 0 24px 0px hsl(var(--sparkle-hue),100%,80%);
        opacity: 0.85;
        animation: sparkle-fade 0.7s cubic-bezier(.7,0,.3,1) forwards;
        z-index: 10;
    }
